...
def search(input_command,x):
    ...
    #Dictionary
        ...
        def micro():
            ...
            receiver = speechsdk.audio.AudioConfig(use_default_microphone=True)
            recognizer = speechsdk.SpeechRecognizer(speech_config=listener, audio_config=receiver)
            window.cout_c("Listening(microsoft)...")
            command = recognizer.recognize_once_async().get()
            if(command.reason == speechsdk.ResultReason.RecognizedSpeech):
                command = command.text
                command = command.strip()
    ...
    #Game
        ...
        if(r==0):
            ...
            def printBoard(board):
                ...
                window.cout_c("-------------------------")
                window.cout_c('   |   |   ')
                window.cout_c(' '+board[1]+' | '+board[2]+' | '+board[3]+' ')
                window.cout_c('   |   |   ')
                window.cout_c('-----------')
                window.cout_c('   |   |   ')
                window.cout_c(' '+board[4]+' | '+board[5]+' | '+board[6]+' ')
                window.cout_c('   |   |   ')
                window.cout_c('-----------')
                window.cout_c('   |   |   ')
                window.cout_c(' '+board[7]+' | '+board[8]+' | '+board[9]+' ')
                window.cout_c('   |   |   ')
                window.cout_c("------------------------")
                window.cout_c("\n")
